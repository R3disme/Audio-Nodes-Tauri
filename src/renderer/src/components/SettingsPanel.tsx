...
const LATENCY_MODES: { id: LatencyMode; label: string; hint: string }[] = [
  ...
  { id: 'balanced', label: 'Balanced', hint: 'Default — a safe margin that still self-tunes down' },
  { id: 'safe', label: 'Safe', hint: 'Largest cushion — most glitch-resistant, highest latency' }
]
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--c-border)' }}>
          <div className="flex items-center gap-2">
            <Sliders size={15} style={{ color: 'var(--c-accent)' }} />
            <span className="font-semibold text-sm">Settings</span>
          </div>
          <div className="flex items-center gap-1">
            {section === 'appearance' && (
              <button onClick={resetTheme} title="Reset theme to default"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="pt-3" style={{ borderTop: '1px solid var(--c-border)' }}>
        <div className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--c-text-dim)' }}>Node scale</div>
...
function AudioSection(): JSX.Element {
  ...
  return (
    <div className="space-y-5">
      {/* Engine */}
      <div>
        <div className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--c-text-dim)' }}>Audio engine</div>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[9px] leading-tight" style={{ color: 'var(--c-text-dim)' }}>switching reloads</span>
        </div>
      </div>

      {/* Latency readout */}
      <div className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--c-text-dim)' }}>
        <Gauge size={13} style={{ color: 'var(--c-accent)' }} />
        <span>Measured latency</span>
        <span className="font-mono ml-auto" style={{ color: 'var(--c-text)' }}>{latency} ms</span>
      </div>

      {/* Latency mode */}
      <div>
        <div className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--c-text-dim)' }}>Latency mode</div>
        {native ? (
          <>
            <div className="flex rounded overflow-hidden ring-1 ring-black/40 text-[10px] font-semibold">
              {LATENCY_MODES.map(m => (
                <button
                  ...
                  style={{
                    background: latencyMode === m.id ? 'var(--c-accent)' : 'var(--c-surface-2)',
                    color: latencyMode === m.id ? '#1a1a1a' : 'var(--c-text-dim)'
                  }}
                >
                  {m.label}
                </button>
            ...
            <p className="text-[10px] mt-2 leading-snug" style={{ color: 'var(--c-text-dim)' }}>
              {LATENCY_MODES.find(m => m.id === latencyMode)?.hint}. The cushion self-tunes within the
              mode's range — it grows after a glitch and slowly shrinks while clean.
            </p>
          </>
        ) : (
          <p className="text-[10px] leading-snug" style={{ color: 'var(--c-text-dim)' }}>
            Web Audio latency is fixed by the browser. Switch to the Native engine to tune it.
          </p>
        )}
      </div>
    </div>
